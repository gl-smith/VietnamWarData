...
basemaps — first as **point maps** of individual incident coordinates,
then as an **aggregate heat map** of incident density.

The basemaps come from the **Google Maps Static API**. Google’s terms
don’t permit redistributing their imagery, so this package does **not**
ship the basemaps — instead,
[`get_satellite_map()`](https://gl-smith.github.io/VietnamWarData/reference/get_satellite_map.md)
fetches them at call time using your own (free) Google Maps API key. The
chunks are not run at build time; the figures shown are pre-rendered
static exports (© Google).

> **Get a key, then cite `ggmap`.** Register a Google Maps Platform API
> key and activate it with `ggmap::register_google(key = "...")` before
> calling
> [`get_satellite_map()`](https://gl-smith.github.io/VietnamWarData/reference/get_satellite_map.md).
> If you use these basemaps, cite Kahle, D. & Wickham, H. (2013),
...
`get_satellite_map("hue")`. Lay incident coordinates over it with
[`geom_point()`](https://ggplot2.tidyverse.org/reference/geom_point.html)
(use `inherit.aes = FALSE` so the points don’t inherit the basemap’s
aesthetics), and facet by month to watch the January 1968 Tet Offensive
unfold.
...
![](figures/satellite-hue-points.png)
...
over a Southeast Asia basemap. Faceting by `aggressor_side` contrasts
where enemy- and friendly-initiated incidents concentrated.
...
![](figures/satellite-se-asia-heatmap.png)

## Notes

- [`get_satellite_map()`](https://gl-smith.github.io/VietnamWarData/reference/get_satellite_map.md)
  also provides a `"saigon"` basemap; for any other area, call
  [`ggmap::get_map()`](https://rdrr.io/pkg/ggmap/man/get_map.html)
  directly with your own bounding box.
- For a province-level static map without satellite imagery, see the
  [province choropleth
  article](https://gl-smith.github.io/VietnamWarData/articles/province-map.md).
- The published, operation-length–weighted versions of these figures
  appear in Smith (2025).
