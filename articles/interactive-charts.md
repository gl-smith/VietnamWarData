...
session required. Drag the year slider (or press play) to watch patterns
change over time; hover for exact values, and click-drag to zoom. The
underlying figures come from the combined incident database
([`get_comb_inc_dta()`](https://gl-smith.github.io/VietnamWarData/reference/get_comb_inc_dta.md)),
pre-aggregated for use here.

## Ground combat charts
...
Enemy incidents by action category — press play or drag the slider to
see how the mix of harassment, terrorism, mining, and attacks shifted
year to year.

Incidents per month — hover for monthly totals; click-drag to zoom into
a period (e.g. the 1968 Tet Offensive).
