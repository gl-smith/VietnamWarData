...
**\#2 Commit** the working copy (clean history/message). *(outward —
your call)*

**\#3 Merge with original repo.** No real git divergence — just commit
`-site` work and push to `gl-smith/VietnamWarData`; then retire the
frozen original folder so there’s one working copy. *(outward — your
call)*

**\#4 Enable GitHub Pages.** `usethis::use_pkgdown_github_pages()`
(creates gh-pages + enables Pages; needs GitHub auth). *(outward — your
call)*
